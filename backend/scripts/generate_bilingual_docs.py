...
Two sets:
  ...
  - "bi" set: English + Sinhala bilingual labels/descriptions
...
L_BI = {
    ...
    "received_from": "Received From / මුදල් ලැබුණේ",
...
SI_STORY = {
    ...
    "counterparty": "කොළඹ සුපර් මාර්ට්",
    "items": [
...
BI_STORY = {
    ...
    "color_po": "#7c3aed", "color_dn": "#ea6c0a", "color_inv": "#2252b5", "color_rec": "#16a34a",
    "counterparty": "Highland Hotels Group / හයිලන්ඩ් හෝටල් සමූහය",
    "items": [
    ...
    "bank_line": "Commercial Bank | Acc: 8809988776 / ගිණුම | Branch: Kandy / ශාඛාව",
}
...
# ════════════════════════════════════════════════════════════════════════════
# Document generators (parameterised by label-dict L and story data S)
# ════════════════════════════════════════════════════════════════════════════

def gen_po(L, S, prefix):
    img, d = new_page()
    color = S["color_po"]
    header_block(d, "Purchase Order" if prefix == "bi" else "මිලදී ගැනීමේ ඇණවුම",
    ...
    y += 18
    text(d, (50, y), S["company_en"] + " / " + S["company_si"], 13, bold=True)
    text(d, (420, y), S["counterparty"], 13, bold=True)
    y += 24
    ...
    text(d, (50, y), L["description"], 11, bold=True, color="#555555")
    text(d, (480, y), L["qty"], 11, bold=True, color="#555555", anchor="ra")
    text(d, (610, y), L["unit_price"], 11, bold=True, color="#555555", anchor="ra")
    text(d, (744, y), L["amount"], 11, bold=True, color="#555555", anchor="ra")
    y += 16; hline(d, y); y += 12
    ...
    text(d, (560, y), L["order_total"], 13, bold=True)
...
def gen_dn(L, S, prefix):
    img, d = new_page()
    color = S["color_dn"]
    header_block(d, "Delivery Note" if prefix == "bi" else "බෙදාහැරීමේ සටහන",
    ...
    y += 26; hline(d, y, color="#dddddd", width=2); y += 18

    text(d, (50, y), L["description"], 11, bold=True, color="#555555")
    text(d, (470, y), L["qty"], 11, bold=True, color="#555555", anchor="ra")
    text(d, (560, y), L["unit"], 11, bold=True, color="#555555", anchor="ra")
    text(d, (744, y), L["condition"], 11, bold=True, color="#555555", anchor="ra")
    y += 16; hline(d, y); y += 12
...
def gen_invoice(L, S, prefix):
    img, d = new_page()
    color = S["color_inv"]
    header_block(d, "Invoice" if prefix == "bi" else "ඉන්වොයිසය",
    ...
    y += 18
    text(d, (50, y), S["counterparty"], 13, bold=True)
    text(d, (420, y), S["company_en"] + " / " + S["company_si"], 13, bold=True)
    y += 22
    ...
    text(d, (50, y), L["description"], 11, bold=True, color="#555555")
    text(d, (520, y), L["qty"], 11, bold=True, color="#555555", anchor="ra")
    text(d, (630, y), L["unit_price"], 11, bold=True, color="#555555", anchor="ra")
    text(d, (744, y), L["amount"], 11, bold=True, color="#555555", anchor="ra")
    y += 16; hline(d, y); y += 12
...
def gen_receipt(L, S, prefix):
    img, d = new_page()
    color = S["color_rec"]
    header_block(d, "Receipt" if prefix == "bi" else "රිසිට්පත",
                 S["rec_no"], DATES[3].strftime("%d %b %Y"), color,
                 S["company_en"], S["company_si"], S["contact"])

    y = 135
    text(d, (50, y), L["received_from"], 11, color="#666666"); y += 18
    text(d, (50, y), S["counterparty"], 15, bold=True); y += 24
    text(d, (50, y), f"{L['po_ref']}: {S['po_no']}  |  {S['inv_no']}", 11, color="#666666"); y += 18
    hline(d, y); y += 16

    text(d, (50, y), L["description"], 11, bold=True, color="#555555")
    text(d, (540, y), L["qty"], 11, bold=True, color="#555555", anchor="ra")
    text(d, (640, y), L["unit_price"], 11, bold=True, color="#555555", anchor="ra")
    text(d, (744, y), L["amount"], 11, bold=True, color="#555555", anchor="ra")
    y += 16; hline(d, y); y += 12
    ...
    text(d, (397, y + 34), f"{S['rec_no']}  ·  {S['company_en']}", 10, color="#166534", anchor="ma")
