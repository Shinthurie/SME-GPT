...
DB_POOL_MIN=             # min pooled DB connections per instance (default 1)
DB_POOL_MAX=             # max pooled DB connections per instance (default 8); instances × this < DB cap
SUPABASE_URL= / SUPABASE_SERVICE_ROLE_KEY= / SUPABASE_STORAGE_BUCKET=  # durable image storage (see docs/SCALING.md)
```
...
- **Scales horizontally on a stateless backend.** No authoritative state lives in the process: in-progress uploads between `/process-document` and `/confirm-save` are in the `processing_sessions` table (migration 010, GC-swept), document images are in Supabase Storage (`storage_client.py`, local disk is only a cache), auth is stateless JWT. Any instance can serve any request. Liveness = `GET /healthz` (no I/O), readiness = `GET /readyz` (DB `SELECT 1`, 503 on failure) — a load balancer must use these, not `/health` (which does LLM checks). DB pool is env-tunable (`DB_POOL_MIN`/`DB_POOL_MAX`, default 1/8); keep `instances × DB_POOL_MAX < DB connection cap`. Postgres + S3-compatible storage means migration is a `pg_dump`/bucket-copy, not a rewrite. Full writeup: [docs/SCALING.md](docs/SCALING.md).
